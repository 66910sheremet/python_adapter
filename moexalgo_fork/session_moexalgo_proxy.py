...
from moexalgo_fork.utils import json, result_deserializer, result_deserializer_stream

BASE_URL = 'http://localhost:24300/iss'
AUTH_URL = 'http://passport.moex.com/authenticate'
AUTH_CERT = None
USE_HTTPS = False
_NEXT_REQUEST_AT = 0
...
class Client(HasOptions):
    ...
    def get_objects(self, path: str, deserializer: Callable[[dict], dict | list], **params) -> Union[list]:
        """
        ...
        async def _async_get_objects(timeout: float) -> Union[list]:
            """
        ...
        def _sync_get_objects(timeout: float) -> Union[list]:
            """
...
def data_gen(path: str,
    ...
    with Session() as client:
        return client.get_objects(path, lambda data: result_deserializer(data, section), **options)


def data_gen_stream(path: str,
                    options: dict,
                    offset: int,
                    limit: int,
                    section: str = 'data') -> Optional[Iterator[dict]]:
